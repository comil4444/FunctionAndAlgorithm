...
/*
 * 索引最小堆
 * 由于data可能是比较大的数据不宜移动，所以给所有的data一个index，最终进行最大堆的排序是通过index排序的(比较数值时候还是按照d*来比较)。将Index排序成最小堆，然后通过index可以定位到原来数据的位置
 * 索引位置从1开始，排除0
 * 0	1	2	3	4	5	6	数组位置
 * 0	1	2	3	4	5	6	索引位置
 ...
 * 0	6	3	1	4	5	2	存放索引位置的初始位置//该位置相当于索引位置的指针，而索引位置相当原数据位置的指针-->该数组的目的是为了可以在更新某个索引数据的时候可以不用遍历而是通过数组直接O(1)来寻找更新，并更新最小堆	
 * 
 * reverse[indexes[i]]=i
 * indexes[reverse[i]]=i
 */
